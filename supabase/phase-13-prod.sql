-- =====================================================================
-- FASE 13 — PRODUÇÃO: superadmin, triggers cross-módulo, índices, realtime
--
...
-- =====================================================================
-- 13.1 SUPERADMIN · flag em profiles
-- Substitui o uso de SK_ADMIN_KEY como segredo compartilhado.
-- Edge functions privilegiadas (provision-tenant) validam essa flag via JWT.
-- =====================================================================
...
-- Helper SQL: chamado pelas edge functions / RLS para checar superadmin
...
comment on function app.is_superadmin is
  'True se o profile do usuário tem is_superadmin = true. Usar em edge functions privilegiadas.';


-- =====================================================================
-- 13.2 INVENTORY → DRE · trigger que gera lançamento ao finalizar inventário
--
-- Quando inventory_sessions vai pra status='finalized' com financial_impact != 0,
-- cria uma finance_entry automática na subcategoria "Ajuste de estoque" daquele
-- tenant (cria a subcategoria se ainda não existir).
--
-- Convenção: o impacto financeiro do inventário é tratado como ajuste contábil
-- — sobra (positivo) reduz custo do mês, perda (negativo) aumenta.
-- =====================================================================
...
begin
  -- Só dispara em transição pra 'finalized' (não em UPDATE pra outros status)
  ...
  -- Resolve a subcategoria "Ajuste de estoque" do tenant; cria se não existir
  ...
  if v_subcat_id is null then
    -- Usa a primeira categoria "Custos" como pai; senão, cria uma sintética
    ...
    values (new.tenant_id, v_category_id, 'Ajuste de estoque', true)
    returning id into v_subcat_id;
  end if;

  -- Idempotência: se já existe entry com auto_source_id = essa sessão, pula
  ...
  -- Cria o lançamento. Sobra = valor positivo (reduz custo, mas registramos
  -- magnitude); perda = valor positivo também — sinal vai no description.
...
-- =====================================================================
-- 13.3 CMV DIÁRIO · função materialize (callable por cron)
--
-- Agrega revenue_entries (faturamento) + stock_movements de saída (cogs real)
-- por (tenant, business_date, operation) e materializa em tabela cmv_daily.
--
-- Por que não calcular on-the-fly? Para períodos grandes o agregado fica caro.
-- A função é idempotente — pode ser chamada N vezes (faz UPSERT).
--
-- Uso:
--   select app.compute_cmv_daily('<tenant_id>'::uuid, current_date - 30, current_date);
-- =====================================================================
...
-- Só edge functions/cron escrevem (sem policy de insert/update pro front)
...
declare
  v_count integer := 0;
begin
  -- Apaga janela existente e re-calcula (atomicamente)
...
comment on function app.compute_cmv_daily is
  'Materializa CMV diário em cmv_daily a partir de revenue_entries no período. Idempotente.';


-- =====================================================================
-- 13.4 ÍNDICES adicionais para queries do dashboard
--
-- Análise dos queries mais hot:
--   - dbListRevenueEntries: filtra por tenant_id + business_date range
--   - dbListStockItems: filtra por tenant_id + is_active
--   - dbTopConsumedItems: stock_movements por tenant + kind='out' + range
--   - dbListKitchenRequests: tenant_id + ordenado por requested_at desc
-- =====================================================================
...
-- =====================================================================
-- 13.5 REALTIME publication · habilita inserts/updates em canais
--
-- Estas tabelas geram eventos que o frontend precisa ver "ao vivo":
--   - kitchen_requests: kanban de requisições já era realtime
--   - stock_movements: novo · refresca o estoque após entrada/saída
--   - revenue_entries: novo · refresca dashboard após lançamento
--   - goods_receipts: novo · notifica conclusão de recebimento
--
-- A publication `supabase_realtime` já existe — só adicionamos as tabelas.
-- =====================================================================

do $$
begin
  -- Garante a publication
  if not exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    create publication supabase_realtime;
  end if;

  -- Adiciona cada tabela se ainda não fizer parte
  if not exists (
...
-- Verificação rápida
select
  (select count(*) from pg_proc where proname = 'is_superadmin')         as fn_superadmin,
  (select count(*) from pg_proc where proname = 'tg_inventory_to_dre')   as fn_inv_to_dre,
  (select count(*) from pg_proc where proname = 'compute_cmv_daily')     as fn_cmv_daily,
