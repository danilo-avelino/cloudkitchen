-- =====================================================================
-- Produção · peso de insumo em unidade não-mássica (2026-08-10)
-- =====================================================================
--
-- PROBLEMA
-- app.production_weight_kg(qty, unit) só converte 'kg' e 'g'; para 'un' devolve
-- NULL. Uma única linha em "un" zera o peso do lote inteiro (v_input_weight
-- vira NULL), e o ramo 'completed' só calcula yield_pct/waste_qty quando
-- input_weight não é nulo. Resultado: quem trabalha com insumo em unidade
-- (calabresa 3 kg, peça de queijo, pacote) nunca vê desperdício — nem na ordem
-- nem no KPI de Análises, que fica em "—".
--
-- SOLUÇÃO
-- O peso de UMA unidade passa a sair de stock_items.portion_qty/portion_unit,
-- os mesmos campos que os transformados já usam para a porção. Reaproveitar em
-- vez de criar coluna nova é proposital: o conceito é o mesmo ("quanto pesa uma
-- unidade deste item") e, quando um transformado é insumo de outra produção, a
-- porção dele JÁ é o peso unitário correto — sem campo duplicado para divergir.
--
-- Item em 'kg' não é afetado: production_weight_kg resolve primeiro e o
-- coalesce nem chega no fallback. Insumo em 'un' sem peso cadastrado continua
-- NULL (mesmo comportamento de antes) — o front avisa para cadastrar.
--
-- ESCOPO
-- Só o ramo draft → issued (peso de entrada). O ramo 'completed' e o rateio
-- multi-saída ficam intocados — inclusive a pendência de religar
-- app.tg_production_apply_complete_alloc(), que segue aberta no DEPLOY-PENDING.
--
-- Base: versão de 20260713120000_production_allow_negative_stock.sql, que é a
-- última a definir esta função. Só `create or replace` — colável por cima.
-- =====================================================================
