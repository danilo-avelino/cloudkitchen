  ...
  const generateAuto = async () => {
    ...
    try {
      ...
      if (source === "db" && tenantId) {
        const bySup = {};
        items.forEach((it) => { (bySup[it.supplier] = bySup[it.supplier] || []).push(it); });
        ...
        await refetchAll();
        window.showToast?.(`Lista gerada · ${items.length} itens`, { tone: "ok" });
      } else {
        ...
        window.showToast?.(`Lista gerada (mock) · ${items.length} itens`, { tone: "warn" });
      }
      ...
      <MobileScroll style={{ padding: "0 14px 12px" }}>
        {enrichedLists.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px 12px", color: "var(--fg-3)", fontSize: 13 }}>
            ...
            <span style={{ fontSize: 11.5 }}>Gere uma sugestão a partir do estoque abaixo do mínimo.</span>
          </div>
        ) : (
...
// ===== Detalhe da lista (fornecedores colapsáveis) =====
function MobilePurchaseDetail({ list, receipts, supplierStatusFor, onBack, onReceive }) {
  const total = list.items.reduce((s, it) => s + (it.est_cost || 0), 0);
  const suppliers = [...new Set(list.items.map((it) => it.supplier))];
  const m = statusMeta(list.computedStatus || list.status);
  return (
          ...
          {suppliers.map((sup) => {
            ...
            return (
              <SupplierCard key={sup} supplier={sup} items={supItems} receipts={supReceipts} status={status}
                            canReceive={status !== "received"} onReceive={() => onReceive(sup)} />
            );
          })}
...
function SupplierCard({ supplier, items, receipts, status, canReceive, onReceive }) {
    ...
    <div style={{ borderRadius: 12, background: "var(--bg-1)", border: "1px solid var(--line)", overflow: "hidden" }}>
      ...
      {open && (
        <div style={{ padding: "0 14px 12px" }}>
          ...
          <MPrimaryButton onClick={onReceive} disabled={!canReceive}>
            <I.Box size={16} />{canReceive ? "Receber mercadoria" : "Tudo recebido"}
          </MPrimaryButton>
        </div>
      )}
    </div>
